...
const { CitaSpa, Terapeuta, ServicioSpa, Usuario } = db;

export const getAll = async () => {
  return await CitaSpa.findAll({
    include: [
      { model: Terapeuta, attributes: ['nombre', 'especialidad'] },
      { model: ServicioSpa, attributes: ['nombre', 'duracion_minutos', 'precio'] },
...
export const getById = async (id: number) => {
  const cita = await CitaSpa.findByPk(id);
...
export const create = async (data: any) => {
  ...
  const servicio = await ServicioSpa.findByPk(data.servicio_spa_id);
  ...
  const traslape = await CitaSpa.findOne({
    where: {
  ...
  // 4. Crear la cita
  const cita = await CitaSpa.create({
    usuario_id: data.usuario_id,
    terapeuta_id: data.terapeuta_id,
    servicio_spa_id: data.servicio_spa_id,
    fecha: data.fecha,
